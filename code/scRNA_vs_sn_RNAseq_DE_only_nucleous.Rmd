...
DefaultAssay(single_nucleous) <- "RNA"

# Normalize RNA data for visualization purposes
Obj.integrated <- NormalizeData(single_nucleous, verbose = FALSE)
...
markers_nucleous <- FindAllMarkers(object = Obj.integrated, 
                          test.use = "wilcox",
                          min.pct = 0.25,
                          min.diff.pct = 0.25,
...
# join annotations
markers_nucleous <- markers_nucleous %>% as_tibble() %>%
    left_join(y = annotations %>% distinct(gene_name, description),
               by = c("gene" = "gene_name")) %>% left_join(cluster_names)

# number of genes per cluster
markers_nucleous %>% group_by(cluster) %>% summarise(n_gene = n())

# save
#write_csv(markers_nucleous, paste0(folder_path,"/Data/markers_nucleous_obj.integ_cs_vs_nc_res0.7.csv"))
...
top10_nucleous <- markers_nucleous %>% group_by(cluster) %>% 
  slice_max(avg_log2FC, n = 10) 
top5_nucleous <- markers_nucleous %>% group_by(cluster) %>% 
...
# number of cluster
clusters.top <- top10_nucleous %>% distinct(cluster) %>% pull()
name.cell <-  top10_nucleous %>% distinct(Cell_Type) %>% pull()

# plot for each cluster the top 10 marker genes based on the avg log2 FC
plot.list.marker.genes <- clusters.top %>% map2(.,name.cell, ~
      top10_nucleous %>% filter(cluster == .x) %>%
...
map2(paste0(folder_path,"/Output/Plots/only_nucleous/top_gene_markers/cluster_",clusters.top,"_vs_rest.tiff"), plot.list.marker.genes, ggsave)
...
Obj.integrated <- ScaleData(Obj.integrated, 
                            features = as.character(unique(top5_nucleous$gene)), 
                            assay = "RNA")
DoHeatmap(Obj.integrated, 
          features = as.character(unique(top5_nucleous_nucleous$gene)), 
          group.by = sel.clust,
          assay = "RNA")

DotPlot(Obj.integrated, 
        features = rev(as.character(unique(top5_nucleous$gene))), 
        group.by = sel.clust,
...
features1 <- top5_nucleous %>% distinct(gene) %>% pull() %>% head(10)

# to stack vnplots -> stack=T , flip = T
#seurat_integrated %>% to name clusters
Obj.integrated %>%
VlnPlot(features = features1, 
        stack=T , 
        flip = T) +
  NoLegend() + ggtitle('Single Nucleous')
